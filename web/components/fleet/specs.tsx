"use client";

import Image from "next/image";
import { useJet } from "@/context/jet";
import type { JSX } from "react";

const Spec = ({
  icon,
  title,
  label,
}: {
  icon: JSX.Element;
  title?: string | number;
  label: string;
}) =>
  title ? (
    <div
      className={
        "flex flex-col justify-center items-center uppercase text-blue-950 fill-blue-950"
      }
    >
      {icon}
      <h2>{title}</h2>
      <p className={"font-light"}>{label}</p>
    </div>
  ) : null;
const Information = ({
  icon,
  title,
  label,
}: {
  icon: JSX.Element;
  title: string;
  label: string;
}) => {
  return (
    <div
      className={
        "w-max flex justify-start items-center text-blue-950 fill-blue-950 gap-[2vw]"
      }
    >
      {icon}
      <div className={"flex flex-col justify-center items-start"}>
        <p className={"font-semibold"}>{title}</p>
        <label
          className={"normal-case"}
          dangerouslySetInnerHTML={{ __html: label }}
        />
      </div>
    </div>
  );
};

const Specs = () => {
  const data = useJet();

  return (
    <section
      className={
        "containerize flex flex-col justify-center items-center text-center mt-[2vh] gap-[4vh] text-blue-950"
      }
    >
      {data.description && <p>{data.description}</p>}
      <div
        className={
          "w-full md:flex grid grid-cols-2 gap-[4vh] justify-around items-center"
        }
      >
        <Spec
          icon={
            <svg
              xmlns="http://www.w3.org/2000/svg"
              x="0px"
              y="0px"
              viewBox="0 0 72 72"
              className={"size-[6vh]"}
            >
              <path d="M37.911,41.902l-7.403,5.842l2.281,7.703c0.281,0.978,0,1.948-0.791,2.738l-0.691,0.691c-1.48,1.482-2.856,0.037-2.893,0	L13.123,43.585c-0.037-0.037-1.483-1.411,0-2.894l0.691-0.691c0.225-0.225,1.223-1.225,2.731-0.792l7.709,2.283l5.842-7.403	l-16.812-6.17c-1.381-0.461-1.668-2.07-0.753-2.987l1.913-1.912c0.048-0.048,0.916-1.042,2.621-0.954l22.966,0.816	c0.827,0.036,1.075-0.048,1.761-0.734l6.506-6.506c1.104-1.104,2.294-1.983,3.54-2.613c1.285-0.65,2.528-0.995,3.694-1.026	c1.99-0.026,2.941,0.888,3.249,1.183c0.851,0.886,1.249,1.979,1.215,3.279c-0.031,1.169-0.378,2.413-1.028,3.697	c-0.629,1.245-1.508,2.435-2.611,3.538l-6.506,6.507c-0.688,0.688-0.774,0.94-0.734,1.756l0.817,22.981	c0.056,1.024-0.274,1.93-0.954,2.611l-1.912,1.912c-0.922,0.926-2.525,0.637-3.002-0.792L37.911,41.902z"></path>
            </svg>
          }
          title={data.images.cabin.day.seats}
          label={"passengers"}
        />
        <Spec
          icon={
            <svg
              xmlns="http://www.w3.org/2000/svg"
              x="0px"
              y="0px"
              viewBox="0 0 72 72"
              className={"size-[6vh]"}
            >
              <path d="M16 20.5c3.584 0 6.5 3.14 6.5 7s-2.916 7-6.5 7-6.5-3.14-6.5-7S12.416 20.5 16 20.5zM24.737 39.415c-6.124 2.962-9.001 7.702-9.562 10.585h-7.59c-2.347 0-3.746-2.109-3.568-3.966C4.561 40.314 9.712 36 16 36 19.445 36 22.544 37.302 24.737 39.415zM66.983 46.026C67.165 47.932 65.74 50 63.409 50c-1.896 0-7.584 0-7.584 0-.564-2.899-3.452-7.63-9.562-10.585C48.455 37.302 51.554 36 55 36 61.282 36 66.439 40.31 66.983 46.026zM55 20.5c3.584 0 6.5 3.14 6.5 7s-2.916 7-6.5 7-6.5-3.14-6.5-7S51.416 20.5 55 20.5zM51.422 49.369C53.326 53.006 50.37 57 46.431 57c-5.465 0-16.396 0-21.861 0-3.782 0-6.969-3.851-4.991-7.63C22.24 44.285 28.488 41 35.5 41S48.761 44.285 51.422 49.369zM35.5 19c5.238 0 9.5 4.486 9.5 10s-4.262 10-9.5 10S26 34.514 26 29 30.262 19 35.5 19z"></path>
            </svg>
          }
          title={Object.values(data.information.crew).reduce(
            (acc, value) => acc + value,
            0,
          )}
          label={"crew"}
        />
        <Spec
          icon={
            <svg
              xmlns="http://www.w3.org/2000/svg"
              x="0px"
              y="0px"
              viewBox="0 0 64 64"
              className={"size-[6vh]"}
            >
              <path d="M 32 10 C 19.85 10 10 19.85 10 32 C 10 44.15 19.85 54 32 54 C 44.15 54 54 44.15 54 32 C 54 19.85 44.15 10 32 10 z M 32 14 C 37.52 14 42.459766 16.490391 45.759766 20.400391 C 44.479766 20.850391 43.399453 21.649141 43.189453 22.619141 C 42.799453 24.409141 43.430469 25.780234 43.980469 26.990234 C 44.460469 28.050234 44.880859 28.959609 44.630859 30.099609 L 44.419922 31.080078 C 44.339922 31.440078 44.47 31.810781 44.75 32.050781 C 45.03 32.290781 45.419766 32.350703 45.759766 32.220703 C 46.329766 32.000703 47.379844 31.720312 47.839844 31.820312 C 48.289844 31.920312 48.430391 32.020703 48.400391 33.470703 C 48.390391 33.640703 48.390625 33.810234 48.390625 33.990234 L 47.900391 33.880859 C 45.550391 33.370859 42.519922 35.110469 42.169922 36.730469 C 41.759922 38.590469 41.990234 41.800703 44.740234 44.720703 C 41.480234 47.980703 36.97 50 32 50 C 22.06 50 14 41.94 14 32 C 14 31.67 14.009531 31.349531 14.019531 31.019531 C 14.989531 31.139531 15.299766 31.240391 15.509766 31.650391 C 15.909766 32.460391 16.170391 33.299062 16.400391 34.039062 C 16.830391 35.419062 17.320156 37 18.910156 37 C 20.980156 37 22.429688 34.100469 22.429688 29.980469 C 22.519688 29.330469 24.159297 27.689609 24.779297 27.599609 C 25.869297 27.599609 26.459844 26.780469 26.839844 26.230469 C 27.309844 25.570469 27.570781 25.259766 28.300781 25.259766 C 31.430781 25.259766 33 22.240547 33 20.560547 C 33 17.880547 31.320547 17.070078 30.310547 16.580078 C 29.480547 16.180078 29.480469 16.119375 29.480469 15.859375 C 29.480469 14.949375 30.009922 14.390781 30.669922 14.050781 C 31.109922 14.020781 31.55 14 32 14 z M 32.808594 35.5 C 32.138086 35.499316 31.444922 35.807813 30.919922 36.039062 C 30.789922 36.099063 30.610469 36.180703 30.480469 36.220703 L 30.470703 36.220703 C 28.760703 35.600703 27.580938 35.980078 26.960938 37.330078 C 26.260938 38.880078 26.360859 41.060781 26.380859 41.300781 L 26.400391 41.460938 C 26.590391 42.300938 27.139922 42.95 27.919922 43.25 C 28.719922 43.56 29.620078 43.439687 30.330078 42.929688 C 30.450078 42.909687 30.799063 43.090313 31.039062 43.320312 C 31.149062 43.430312 31.990547 44.240234 33.060547 44.240234 C 33.250547 44.240234 33.450391 44.210391 33.650391 44.150391 C 34.620391 43.860391 35.279609 42.910313 35.599609 41.320312 C 36.199609 38.430312 34.829219 36.689922 33.949219 35.919922 C 33.604219 35.612422 33.210898 35.50041 32.808594 35.5 z"></path>
            </svg>
          }
          title={`${data.information.range.toLocaleString()} km`}
          label={"range"}
        />
        <Spec
          icon={
            <svg
              xmlns="http://www.w3.org/2000/svg"
              x="0px"
              y="0px"
              viewBox="0 0 72 72"
              className={"size-[6vh]"}
            >
              <path d="M 36.015625 15.25 C 32.178625 15.25 28.404406 15.786375 24.691406 16.859375 C 20.978406 17.932375 17.548344 19.453828 14.402344 21.423828 C 11.256344 23.393828 8.6117969 25.708141 6.4667969 28.369141 C 6.1367969 28.720141 5.9809531 29.138047 6.0019531 29.623047 C 6.0229531 30.108047 6.2188438 30.525953 6.5898438 30.876953 L 10.210938 34.589844 C 10.602938 35.001844 11.083391 35.201406 11.650391 35.191406 C 12.217391 35.181406 12.697844 34.960344 13.089844 34.527344 C 16.162844 31.309344 19.637625 28.864359 23.515625 27.193359 C 27.394625 25.522359 31.561578 24.6875 36.017578 24.6875 C 40.493578 24.6875 44.676453 25.522359 48.564453 27.193359 C 52.452453 28.864359 55.933813 31.330844 59.007812 34.589844 C 59.378813 34.981844 59.841437 35.167484 60.398438 35.146484 C 60.955437 35.125484 61.430266 34.919344 61.822266 34.527344 L 65.441406 30.876953 C 65.792406 30.525953 65.977047 30.108047 65.998047 29.623047 C 66.019047 29.138047 65.875406 28.720141 65.566406 28.369141 C 63.421406 25.707141 60.769281 23.393828 57.613281 21.423828 C 54.457281 19.453828 51.023547 17.932375 47.310547 16.859375 C 43.597547 15.786375 39.831625 15.25 36.015625 15.25 z M 36.015625 30.410156 C 33.437625 30.410156 30.926422 30.736719 28.482422 31.386719 C 26.037422 32.036719 23.783703 32.958297 21.720703 34.154297 C 19.657703 35.350297 17.915187 36.775781 16.492188 38.425781 C 16.162188 38.776781 16.011969 39.168563 16.042969 39.601562 C 16.073969 40.034562 16.275484 40.426344 16.646484 40.777344 L 20.699219 44.521484 C 21.132219 44.913484 21.621922 45.10475 22.169922 45.09375 C 22.716922 45.08375 23.207625 44.850484 23.640625 44.396484 C 25.249625 42.766484 27.152609 41.499797 29.349609 40.591797 C 31.546609 39.684797 33.769578 39.230469 36.017578 39.230469 C 38.286578 39.230469 40.533719 39.699672 42.761719 40.638672 C 44.989719 41.576672 46.918828 42.871484 48.548828 44.521484 C 48.919828 44.933484 49.368531 45.133047 49.894531 45.123047 C 50.420531 45.113047 50.899031 44.913484 51.332031 44.521484 L 55.386719 40.777344 C 55.736719 40.426344 55.926031 40.034562 55.957031 39.601562 C 55.988031 39.168563 55.848062 38.776781 55.539062 38.425781 C 54.095063 36.775781 52.342297 35.350297 50.279297 34.154297 C 48.216297 32.958297 45.963531 32.036719 43.519531 31.386719 C 41.075531 30.736719 38.573625 30.410156 36.015625 30.410156 z M 36.046875 44.677734 C 34.313875 44.677734 32.606781 45.083438 30.925781 45.898438 C 29.243781 46.713438 27.846422 47.864609 26.732422 49.349609 C 26.485422 49.679609 26.41825 50.055516 26.53125 50.478516 C 26.64425 50.900516 26.857016 51.267172 27.166016 51.576172 L 33.353516 57.548828 C 33.910516 58.064828 34.390969 58.435109 34.792969 58.662109 C 35.194969 58.889109 35.613875 59.001953 36.046875 59.001953 C 36.459875 59.001953 36.871156 58.887109 37.285156 58.662109 C 37.697156 58.435109 38.181281 58.064828 38.738281 57.548828 L 44.896484 51.576172 C 45.247484 51.225172 45.443375 50.803594 45.484375 50.308594 C 45.525375 49.813594 45.380781 49.370516 45.050781 48.978516 C 43.957781 47.637516 42.607047 46.585266 40.998047 45.822266 C 39.389047 45.059266 37.737875 44.677734 36.046875 44.677734 z"></path>
            </svg>
          }
          title={data.information.wifi ? "wifi" : undefined}
          label={"on board"}
        />
      </div>
      {data.brochure && (
        <a
          href={`${process.env.NEXT_PUBLIC_PAYLOAD_URL}${data.brochure.url}`}
          target={"_blank"}
          className={"glass-button glass-button-dark z-0 my-[8vh]"}
        >
          Download fact sheet
        </a>
      )}
      <div className={"w-full flex flex-col justify-between items-start"}>
        <div
          className={
            "w-full flex justify-between items-baseline text-blue-950 gap-[8vw] py-0"
          }
        >
          <label className={"w-1/2 text-left hidden md:block"}>
            &mdash; information
          </label>
          <label className={"w-1/2 text-left hidden md:block"}>
            &mdash; cabin layout
          </label>
        </div>
        <div
          className={
            "w-full flex flex-col md:flex-row justify-between items-center gap-[8vw]"
          }
        >
          <label className={"w-full text-left md:hidden"}>
            &mdash; information
          </label>
          <div
            className={
              "md:w-1/2 w-full grid lg:grid-cols-2 grid-cols-1 gap-y-[2vh] gap-x-[4vh]"
            }
          >
            <Information
              icon={
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  x="0px"
                  y="0px"
                  viewBox="0 0 72 72"
                  className={"size-[4vh]"}
                >
                  <path d="M55,40c2.209,0,4,1.791,4,4s-1.791,4-4,4h-7v7c0,2.209-1.791,4-4,4s-4-1.791-4-4v-7h-8v7c0,2.209-1.791,4-4,4s-4-1.791-4-4	v-7h-7c-2.209,0-4-1.791-4-4s1.791-4,4-4h7v-8h-7c-2.209,0-4-1.791-4-4s1.791-4,4-4h7v-7c0-2.209,1.791-4,4-4s4,1.791,4,4v7h8v-7	c0-2.209,1.791-4,4-4s4,1.791,4,4v7h7c2.209,0,4,1.791,4,4s-1.791,4-4,4h-7v8H55z M32,40h8v-8h-8V40z"></path>
                </svg>
              }
              title={"Cabin size"}
              label={`L ${data.information.cabin.length} m, W ${data.information.cabin.width} m, H ${data.information.cabin.height} m`}
            />
            <Information
              icon={
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  x="0px"
                  y="0px"
                  viewBox="0 0 72 72"
                  className={"size-[4vh]"}
                >
                  <path d="M16 20.5c3.584 0 6.5 3.14 6.5 7s-2.916 7-6.5 7-6.5-3.14-6.5-7S12.416 20.5 16 20.5zM24.737 39.415c-6.124 2.962-9.001 7.702-9.562 10.585h-7.59c-2.347 0-3.746-2.109-3.568-3.966C4.561 40.314 9.712 36 16 36 19.445 36 22.544 37.302 24.737 39.415zM66.983 46.026C67.165 47.932 65.74 50 63.409 50c-1.896 0-7.584 0-7.584 0-.564-2.899-3.452-7.63-9.562-10.585C48.455 37.302 51.554 36 55 36 61.282 36 66.439 40.31 66.983 46.026zM55 20.5c3.584 0 6.5 3.14 6.5 7s-2.916 7-6.5 7-6.5-3.14-6.5-7S51.416 20.5 55 20.5zM51.422 49.369C53.326 53.006 50.37 57 46.431 57c-5.465 0-16.396 0-21.861 0-3.782 0-6.969-3.851-4.991-7.63C22.24 44.285 28.488 41 35.5 41S48.761 44.285 51.422 49.369zM35.5 19c5.238 0 9.5 4.486 9.5 10s-4.262 10-9.5 10S26 34.514 26 29 30.262 19 35.5 19z"></path>
                </svg>
              }
              title={"Crew"}
              label={`${data.information.crew.pilots} Pilot${data.information.crew.pilots > 1 ? "s" : ""}, ${data.information.crew.attendants} Flight Attendant${data.information.crew.attendants > 1 ? "s" : ""}`}
            />
            <Information
              icon={
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  x="0px"
                  y="0px"
                  viewBox="0 0 72 72"
                  className={"size-[4vh]"}
                >
                  <path d="M50,61c-0.731,0-1.464-0.2-2.112-0.604L36,53.004l-11.888,7.393c-1.356,0.847-3.088,0.799-4.401-0.116	c-1.312-0.916-1.952-2.524-1.628-4.091l2.923-14.118l-10.631-9.246c-1.203-1.047-1.67-2.708-1.188-4.228	c0.481-1.52,1.819-2.608,3.406-2.771l14.115-1.442L32.33,11.41C32.964,9.947,34.405,9,36,9s3.036,0.947,3.67,2.41l5.621,12.975	l14.115,1.442c1.587,0.162,2.925,1.251,3.406,2.771c0.482,1.52,0.016,3.181-1.188,4.228l-10.631,9.246l2.923,14.118	c0.324,1.566-0.316,3.175-1.628,4.091C51.604,60.759,50.802,61,50,61z"></path>
                </svg>
              }
              title={"Maximum speed"}
              label={`${data.information.speed.toLocaleString()} km | ${Number((data.information.speed * 0.539957).toFixed(0)).toLocaleString()} nmi`}
            />
            <Information
              icon={
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  x="0px"
                  y="0px"
                  viewBox="0 0 64 64"
                  className={"size-[4vh]"}
                >
                  <path d="M 32 10 C 19.85 10 10 19.85 10 32 C 10 44.15 19.85 54 32 54 C 44.15 54 54 44.15 54 32 C 54 19.85 44.15 10 32 10 z M 32 14 C 37.52 14 42.459766 16.490391 45.759766 20.400391 C 44.479766 20.850391 43.399453 21.649141 43.189453 22.619141 C 42.799453 24.409141 43.430469 25.780234 43.980469 26.990234 C 44.460469 28.050234 44.880859 28.959609 44.630859 30.099609 L 44.419922 31.080078 C 44.339922 31.440078 44.47 31.810781 44.75 32.050781 C 45.03 32.290781 45.419766 32.350703 45.759766 32.220703 C 46.329766 32.000703 47.379844 31.720312 47.839844 31.820312 C 48.289844 31.920312 48.430391 32.020703 48.400391 33.470703 C 48.390391 33.640703 48.390625 33.810234 48.390625 33.990234 L 47.900391 33.880859 C 45.550391 33.370859 42.519922 35.110469 42.169922 36.730469 C 41.759922 38.590469 41.990234 41.800703 44.740234 44.720703 C 41.480234 47.980703 36.97 50 32 50 C 22.06 50 14 41.94 14 32 C 14 31.67 14.009531 31.349531 14.019531 31.019531 C 14.989531 31.139531 15.299766 31.240391 15.509766 31.650391 C 15.909766 32.460391 16.170391 33.299062 16.400391 34.039062 C 16.830391 35.419062 17.320156 37 18.910156 37 C 20.980156 37 22.429688 34.100469 22.429688 29.980469 C 22.519688 29.330469 24.159297 27.689609 24.779297 27.599609 C 25.869297 27.599609 26.459844 26.780469 26.839844 26.230469 C 27.309844 25.570469 27.570781 25.259766 28.300781 25.259766 C 31.430781 25.259766 33 22.240547 33 20.560547 C 33 17.880547 31.320547 17.070078 30.310547 16.580078 C 29.480547 16.180078 29.480469 16.119375 29.480469 15.859375 C 29.480469 14.949375 30.009922 14.390781 30.669922 14.050781 C 31.109922 14.020781 31.55 14 32 14 z M 32.808594 35.5 C 32.138086 35.499316 31.444922 35.807813 30.919922 36.039062 C 30.789922 36.099063 30.610469 36.180703 30.480469 36.220703 L 30.470703 36.220703 C 28.760703 35.600703 27.580938 35.980078 26.960938 37.330078 C 26.260938 38.880078 26.360859 41.060781 26.380859 41.300781 L 26.400391 41.460938 C 26.590391 42.300938 27.139922 42.95 27.919922 43.25 C 28.719922 43.56 29.620078 43.439687 30.330078 42.929688 C 30.450078 42.909687 30.799063 43.090313 31.039062 43.320312 C 31.149062 43.430312 31.990547 44.240234 33.060547 44.240234 C 33.250547 44.240234 33.450391 44.210391 33.650391 44.150391 C 34.620391 43.860391 35.279609 42.910313 35.599609 41.320312 C 36.199609 38.430312 34.829219 36.689922 33.949219 35.919922 C 33.604219 35.612422 33.210898 35.50041 32.808594 35.5 z"></path>
                </svg>
              }
              title={"Maximum range"}
              label={`${data.information.range.toLocaleString()} km | ${Number((data.information.range * 0.539957).toFixed(0)).toLocaleString()} nmi`}
            />
            <Information
              icon={
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  x="0px"
                  y="0px"
                  viewBox="0 0 64 64"
                  className={"size-[4vh]"}
                >
                  <path d="M32,10c12.15,0,22,9.85,22,22s-9.85,22-22,22s-22-9.85-22-22S19.85,10,32,10z M34,32c0-0.366,0-13.634,0-14	c0-1.105-0.896-2-2-2s-2,0.895-2,2c0,0.282,0,8.196,0,12c-2.66,0-9.698,0-10,0c-1.105,0-2,0.896-2,2s0.895,2,2,2	c0.366,0,11.826,0,12,0C33.104,34,34,33.105,34,32z"></path>
                </svg>
              }
              title={"Flight Hours"}
              label={`${Math.floor(data.information.hours)} h ${Math.round((data.information.hours % 1) * 60)} min`}
            />
            <Information
              icon={
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  x="0px"
                  y="0px"
                  viewBox="0 0 72 72"
                  className={"size-[4vh]"}
                >
                  <path d="M 32 9 C 28.686 9 26 11.686 26 15 L 26 17 L 26 57 L 46 57 L 46 17 L 46 15 C 46 11.686 43.314 9 40 9 L 32 9 z M 32 13 L 40 13 C 41.104 13 42 13.895 42 15 L 30 15 C 30 13.895 30.896 13 32 13 z M 18 15 C 13.582 15 10 18.582 10 23 L 10 49 C 10 53.418 13.582 57 18 57 L 22 57 L 22 15 L 18 15 z M 50 15 L 50 57 L 54 57 C 58.418 57 62 53.418 62 49 L 62 23 C 62 18.582 58.418 15 54 15 L 50 15 z"></path>
                </svg>
              }
              title={"Baggage"}
              label={`${data.information.baggage} m<sup>3</sup>`}
            />
          </div>
          <div
            className={
              "md:w-1/2 w-full flex flex-col gap-[2vh] md:gap-0 justify-center items-center text-blue-950"
            }
          >
            <label className={"w-full text-left md:hidden"}>
              &mdash; cabin layout
            </label>
            <p>Day: {data.images.cabin.day.seats} seats</p>
            <Image
              src={`${process.env.NEXT_PUBLIC_PAYLOAD_URL}${data.images.cabin.day.image.url}`}
              width={data.images.cabin.day.image.width}
              height={data.images.cabin.day.image.height}
              className={"w-full h-auto"}
              alt={data.images.cabin.day.image.alt}
            />
            <p>Night: {data.images.cabin.night.beds} beds</p>
            <Image
              src={`${process.env.NEXT_PUBLIC_PAYLOAD_URL}${data.images.cabin.night.image.url}`}
              width={data.images.cabin.night.image.width}
              height={data.images.cabin.night.image.height}
              className={"w-full h-auto"}
              alt={data.images.cabin.night.image.alt}
            />
          </div>
        </div>
      </div>
    </section>
  );
};

export default Specs;
